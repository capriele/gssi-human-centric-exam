                    ...
                    if self.currentStep.hasBeenAnswered():
                        ...
                        if patient is not None:
                            ...
                            if answer:
                                ...
                                patientHumorConfiguration = self.robot.configuration.get_properties().get_property()[
                                    0]

                                patientHumor = self.planConditioners["pill_attempts"].random(
                                    patientHumorConfiguration.from_, patientHumorConfiguration.to)
                                Logger.i(
                                    f"Patient Humor Score: {patientHumor}")

                                self.planConditioners["pill_attempts"].inc()
                                Logger.i(
                                    f"Number of attempts: {self.planConditioners['pill_attempts'].status + 1}")
                                if (self.planVerificators["patient_humor"].verify(["patient_humor_good", "patient_humor_bad"], patientHumor, self.planConditioners["pill_attempts"].status)):
                                    self.steps.insert(
                                        0,
                                        PlanStep(
                                            None, Activity.WAIT_ANSWER, status=f"Attempt number {self.planConditioners['pill_attempts'].status + 1}. {patient.name}, can I enter?", patient=patient),
                                    )
                                else:
                                    self.planConditioners["pill_attempts"].reset(
                                    )
